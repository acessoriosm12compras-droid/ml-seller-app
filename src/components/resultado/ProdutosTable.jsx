...
function MargemBadge({ value }) {
  if (value === null || value === undefined) return <span className="text-stone-400 dark:text-stone-600">—</span>
...
export default function ProdutosTable({ produtos, titulo = 'top-produtos' }) {
  ...
  return (
    <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden">
      <div className="flex justify-end px-4 py-2 border-b border-stone-200 dark:border-stone-800">
        <button
          onClick={handleExport}
          className="flex items-center gap-1.5 text-xs text-stone-500 dark:text-stone-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors"
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-stone-100 dark:bg-stone-800/50 text-stone-600 dark:text-stone-400 border-b border-stone-200 dark:border-stone-800">
              {COLUMNS.map(col => (
                <th
                  key={col.key}
                  onClick={() => handleSort(col.key)}
                  className={`px-4 py-3 font-semibold cursor-pointer select-none hover:text-stone-900 dark:hover:text-stone-200 transition-colors ${col.align === 'right' ? 'text-right' : col.align === 'center' ? 'text-center' : 'text-left'}`}
                >
          ...
          <tbody>
            {sorted.map((p) => (
              <tr key={p.ml_item_id} className="border-b border-stone-100 dark:border-stone-800/50 hover:bg-stone-50 dark:hover:bg-stone-800/30">
                {/* Produto */}
                <td className="px-4 py-3">
                  <p className="text-stone-800 dark:text-stone-200 truncate max-w-[200px]">{p.titulo}</p>
                  <p className="text-stone-400 dark:text-stone-600 text-xs">{p.ml_item_id}</p>
                </td>
                {/* Preço Médio */}
                ...
                <td className="px-4 py-3 text-right">
                  {p.custo_unitario === null || p.custo_unitario === undefined
                    ? <span className="text-red-500 dark:text-red-400 text-xs">sem custo</span>
                    : <span className="text-stone-700 dark:text-stone-300">{formatBRL(p.custo_unitario)}</span>
                  }
                </td>
                {/* Unidades */}
                <td className="px-4 py-3 text-right text-stone-700 dark:text-stone-300">
                ...
                <td className="px-4 py-3 text-right">
                  ...
                  {p.representatividade !== null && p.representatividade !== undefined && (
                    <div className="mt-1 h-0.5 bg-stone-200 dark:bg-stone-800 rounded-full w-full">
                      <div
                ...
                <td className="px-4 py-3 text-right">
                  {p.lucro === null || p.lucro === undefined
                    ...
                    : <span className={p.lucro >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}>
                        {formatBRL(p.lucro)}
                      </span>
                  }
                </td>
                ...
                <td className="px-4 py-3 text-right">
                  {p.lucro_pos_ads === null || p.lucro_pos_ads === undefined
                    ? <span className="text-stone-400 dark:text-stone-600">—</span>
                    : <span className={p.lucro_pos_ads >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}>

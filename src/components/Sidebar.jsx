...
import {
  ...
  Wallet, ArrowDownCircle, ArrowUpCircle, Building2, TrendingUp, GitMerge, Receipt, Tag,
} from 'lucide-react'
        ...
        <nav className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden py-3 flex flex-col gap-0.5 px-2">
          {NAV.map(({ to, label, icon: Icon }) => (
            <NavLink key={to} to={to} title={label} className={itemClass}>
              {({ isActive }) => (
                <>
                  <Icon size={16} strokeWidth={1.75} className={`shrink-0 ${isActive ? 'text-accent-text' : ''}`} />
          ...
          <div className="mt-0.5">
            ...
            >
              ...
              <span className="flex-1 text-sm whitespace-nowrap text-left">Financeiro</span>
              {finOpen
            ...
            {finOpen && (
              <div className="mt-0.5 flex flex-col gap-0.5">
                {FIN_NAV.map(({ to, label, icon: Icon }) => (
                  <NavLink key={to} to={to} title={label} className={subItemClass}>
                    {({ isActive }) => (
                      <>
                        <Icon size={13} strokeWidth={1.75} className={`shrink-0 ${isActive ? 'text-accent-text' : ''}`} />

...
function TendenciaBadge({ t }) {
  ...
  const cfg = {
    ...
    estavel: { color: 'text-gray-400 bg-gray-700/50', icon: '●', label: 'Estável' },
    queda: { color: 'text-red-400 bg-red-500/10', icon: '▼', label: 'Queda' },
  }[t] || { color: 'text-gray-500', icon: '—', label: t }
  return (
      ...
      <main className="flex-1 p-6 overflow-auto space-y-4">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-800">
                    ...
                    <th className="px-4 py-3 text-right text-gray-500">Taxas ML</th>
                    <th className="px-4 py-3 text-right text-gray-500">Imposto</th>
                    <th className="px-4 py-3 text-right text-gray-500">Custo ADS</th>
                    <th className="px-4 py-3 text-right text-gray-500">Custo Prod.</th>
                    <th className="px-4 py-3 text-right text-gray-500">Lucro</th>
                ...
                <tbody>
                  {loadProd && (
                    <tr><td colSpan={11} className="px-4 py-10 text-center text-gray-600">Carregando…</td></tr>
                  )}
                  {!loadProd && produtos.length === 0 && (
                    <tr><td colSpan={11} className="px-4 py-10 text-center text-gray-600">Sem dados no período.</td></tr>
                  )}
                  {produtos.map((p, i) => {
                    const lucroColor = p.lucro === null ? 'text-gray-500' : p.lucro >= 0 ? 'text-emerald-400' : 'text-red-400'
                    const mpaColor = p.mpa === null ? 'text-gray-500' : p.mpa >= 15 ? 'text-emerald-400' : p.mpa >= 0 ? 'text-amber-400' : 'text-red-400'
                    return (
                      <tr key={p.ml_item_id || i} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right text-gray-400">{formatBRL(p.imposto)}</td>
                        <td className="px-4 py-3 text-right text-gray-400">{formatBRL(p.custo_ads)}</td>
                        ...
                        <td className={`px-4 py-3 text-right font-semibold ${lucroColor}`}>{p.lucro !== null ? formatBRL(p.lucro) : <span className="text-gray-600">—</span>}</td>
                        <td className={`px-4 py-3 text-right font-medium ${mpaColor}`}>{p.mpa !== null ? formatPct(p.mpa) : <span className="text-gray-600">—</span>}</td>
                        <td className={`px-4 py-3 text-right ${p.roi >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{p.roi !== null ? formatPct(p.roi) : <span className="text-gray-600">—</span>}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        {tab === 'anuncios' && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-800">
              <h2 className="text-sm font-semibold text-gray-200">
                Período: <span className="text-gray-400">{anuncData?.periodo_atual || '—'}</span>
                <span className="mx-2 text-gray-700">vs</span>
                <span className="text-gray-400">{anuncData?.periodo_anterior || '—'}</span>
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-800">
                    ...
                    <th className="px-4 py-3 text-right text-gray-500">Fat. Atual</th>
                    <th className="px-4 py-3 text-right text-gray-500">Fat. Anterior</th>
                    <th className="px-4 py-3 text-right text-gray-500">Variação</th>
                    <th className="px-4 py-3 text-center text-gray-500">Tendência</th>
                    <th className="px-4 py-3 text-right text-gray-500">Un. Atual</th>
                    <th className="px-4 py-3 text-right text-gray-500">Un. Anterior</th>
                  </tr>
                </thead>
                <tbody>
                  {loadAnunc && (
                    <tr><td colSpan={7} className="px-4 py-10 text-center text-gray-600">Carregando…</td></tr>
                  )}
                  {!loadAnunc && anuncios.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-10 text-center text-gray-600">Sem dados no período.</td></tr>
                  )}
                  {anuncios.map((a, i) => {
                    const varPct = a.variacao_faturamento
                    const varColor = varPct === null ? 'text-gray-500' : varPct > 0 ? 'text-emerald-400' : varPct < 0 ? 'text-red-400' : 'text-gray-400'
                    return (
                      <tr key={a.ml_item_id || i} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        <td className="px-4 py-3 text-gray-300 max-w-[220px]">
                          <div className="truncate" title={a.titulo}>{a.titulo}</div>
                          <div className="text-gray-600 font-mono text-[10px]">{a.ml_item_id}</div>
                        </td>
                        <td className="px-4 py-3 text-right text-amber-400 font-medium">{formatBRL(a.faturamento_atual)}</td>
                        <td className="px-4 py-3 text-right text-gray-500">{formatBRL(a.faturamento_anterior)}</td>
                        ...
                        <td className="px-4 py-3 text-right text-gray-300">{a.unidades_atual}</td>
                        <td className="px-4 py-3 text-right text-gray-500">{a.unidades_anterior}</td>
                      </tr>
                    )
                  })}
                </tbody>

...
function KpiRow({ label, value, valueColor = 'text-stone-200', indent = false, bold = false }) {
  return (
    <div className={`flex items-center justify-between py-2.5 border-b border-stone-800/50 ${indent ? 'pl-4' : ''}`}>
      <span className={`text-xs ${indent ? 'text-stone-500' : 'text-stone-400'}`}>{label}</span>
          ...
          <>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
              ...
              <div className="bg-stone-900 border border-stone-800 rounded-xl p-5">
                <h2 className="text-sm font-semibold text-stone-200 mb-4">DRE Simplificada — {d?.mes || mes}</h2>

                {loadM ? (
                  <p className="text-stone-600 text-sm text-center py-8">Carregando…</p>
                ) : d ? (
                  <div>
                    <KpiRow label="Faturamento Bruto" value={formatBRL(d.faturamento)} valueColor="text-sky-400" bold />
                    <KpiRow label="Deduções ML (total)" value={`-${formatBRL(d.deducoes_ml?.total)}`} valueColor="text-red-400" />
                    <KpiRow label="Comissão + Taxa fixa" value={`-${formatBRL(d.deducoes_ml?.comissao_taxa_fixa)}`} valueColor="text-stone-500" indent />
                    <KpiRow label="Frete + Outros" value={`-${formatBRL(d.deducoes_ml?.frete_outros)}`} valueColor="text-stone-500" indent />
                    <KpiRow label="Líq. do Marketplace" value={formatBRL(d.liquido_marketplace)} valueColor="text-blue-400" bold />
                    <KpiRow label="Imposto" value={`-${formatBRL(d.imposto)}`} valueColor="text-stone-500" />
                    <KpiRow label="Custo dos Produtos (CMV)" value={`-${formatBRL(d.custo_produtos)}`} valueColor="text-stone-500" />
                    <KpiRow
                      label="Lucro Bruto"
                      ...
                      bold
                    />
                    <KpiRow label={`Margem Bruta`} value={formatPct(d.margem_bruta)} valueColor={d.margem_bruta >= 15 ? 'text-emerald-400' : d.margem_bruta >= 0 ? 'text-sky-400' : 'text-red-400'} indent />
                    <KpiRow label="ADS (Publicidade)" value={`-${formatBRL(d.valor_ads)}`} valueColor="text-stone-500" />
                    <KpiRow
                      label="Lucro pós ADS"
                      value={formatBRL(d.lucro_pos_ads)}
                      valueColor={d.lucro_pos_ads >= 0 ? 'text-emerald-400' : 'text-red-400'}
                    />
                    <KpiRow label="Despesas Operacionais" value={`-${formatBRL(d.despesas_operacionais)}`} valueColor="text-stone-500" />
                    <KpiRow label="Receitas Extras" value={formatBRL(d.receitas_extras)} valueColor="text-stone-400" />
                    <KpiRow
                      ...
                      bold
                    />
                    <KpiRow label="Margem Operacional" value={formatPct(d.margem_operacional)} valueColor={d.margem_operacional >= 15 ? 'text-emerald-400' : d.margem_operacional >= 0 ? 'text-sky-400' : 'text-red-400'} indent />
                  </div>
                ) : null}
              </div>
              ...
              <div className="space-y-4">
                ...
                {d && (
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <div key={k.label} className="bg-stone-900 border border-stone-800 rounded-xl px-4 py-3">
                        <p className="text-xs text-stone-500">{k.label}</p>
                        <p className="text-lg font-semibold text-stone-200">{k.value || '—'}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {d?.despesas_por_categoria?.length > 0 && (
                  <div className="bg-stone-900 border border-stone-800 rounded-xl p-5">
                    <h3 className="text-sm font-semibold text-stone-200 mb-3">Despesas por Categoria</h3>
                    <div className="space-y-2">
                      {d.despesas_por_categoria.map(c => (
                        <div key={c.categoria} className="flex items-center justify-between">
                          <span className="text-xs text-stone-400">{c.categoria}</span>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-stone-800">
                      <th className="px-4 py-3 text-left text-stone-500">Mês</th>
                      <th className="px-4 py-3 text-right text-stone-500">Despesas</th>
                      <th className="px-4 py-3 text-right text-stone-500">Receitas Extras</th>
                      <th className="px-4 py-3 text-right text-stone-500">Saldo</th>
                    </tr>
                  </thead>
                  <tbody>
                    {meses.map(m => {
                      ...
                      return (
                        <tr key={m.mes} className="border-b border-stone-800/50 hover:bg-stone-800/30">
                          <td className="px-4 py-3 text-stone-300 font-medium capitalize">{m.nome}</td>

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Empresa / Fornecedor</label>
              <input
                type="text"
                value={form.empresa}
          ...
          {!isEditing && (
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs text-gray-400">Lançamento recorrente</span>
              </label>
              {form.recorrente && (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-16 bg-gray-800 border border-gray-700 rounded-lg px-2 py-1 text-xs text-gray-200 focus:outline-none focus:ring-1 focus:ring-amber-500"
                  />
      ...
      <main className="flex-1 p-6 space-y-5 overflow-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg text-sm font-medium text-black bg-amber-400 hover:bg-amber-300 transition-colors"
          >
            + Nova movimentação
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className={`bg-gray-900 border border-gray-800 border-l-4 ${saldo >= 0 ? 'border-l-blue-500' : 'border-l-red-500'} rounded-xl p-4`}>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  <th className="px-4 py-3 text-left text-gray-500">Categoria</th>
                  <th className="px-4 py-3 text-left text-gray-500">Empresa</th>
                  <th className="px-4 py-3 text-left text-gray-500">Descrição</th>
                  <th className="px-4 py-3 text-right text-gray-500">Valor</th>
                  <th className="px-4 py-3 text-right text-gray-500">Data</th>
                  <th className="px-4 py-3 text-center text-gray-500">Ações</th>
                </tr>
              </thead>
              <tbody>
                {isLoading && (
                ...
                {items.map(item => (
                  <tr key={item.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-400">{item.empresa || '—'}</td>
                    <td className="px-4 py-3 text-gray-400 max-w-[180px] truncate">{item.descricao || '—'}</td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {deletingId === item.id ? (
                          <div className="flex gap-1">
                            ...
                            <button onClick={() => setDeletingId(null)} className="text-gray-500 text-xs">Cancelar</button>
                          </div>
                        ) : (
                          <button
